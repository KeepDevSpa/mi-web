    ...
    const fetchData = async () => {
      try {
        ...
        if (heroRes.ok) {
          ...
          if (activeConfig) {
            console.log("Hero config loaded:", activeConfig);
            console.log("Hero image:", activeConfig.heroImage);
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <EnhancedHero 
        ...
        image={heroConfig?.heroImage || "/Hero-Banner.webp"} // Usamos una imagen que sabemos que existe
        variant={heroConfig?.variant || "default"}
        overlayOpacity={0.8} // Aumentamos la opacidad para que la imagen sea más visible
        primaryCta={{
          text: heroConfig?.primaryCta?.text || "Ver ofertas",
          href: heroConfig?.primaryCta?.href || "#ofertas",
          style: "secondary"
        }}
        secondaryCta={{
          text: heroConfig?.secondaryCta?.text || "Comprobar cobertura",
          href: heroConfig?.secondaryCta?.href || "#coverage-form",
          style: "outline"
        }}
        highlights={heroConfig?.highlights || [
          { text: "Fibra hasta 1Gbps", style: "badge", className: "bg-gradient-to-r from-[#00aa00]/60 to-[#008800]/60 hover:from-[#00aa00]/80 hover:to-[#008800]/80 backdrop-blur-md transition" },
          { text: "Móvil con 5G", style: "badge", className: "bg-gradient-to-r from-[#0066cc]/60 to-[#0055aa]/60 hover:from-[#0066cc]/80 hover:to-[#0055aa]/80 backdrop-blur-md transition" },
          { text: "Sin permanencia", style: "badge", className: "bg-gradient-to-r from-amber-500/60 to-amber-600/60 hover:from-amber-500/80 hover:to-amber-600/80 backdrop-blur-md transition" }
        ]}
